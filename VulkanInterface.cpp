...
void VulkanInterface::InitializeVulkan()
{
    ...
    auto temp = FACTORY(RenderObject).GetRegisteredClasses();
...
void VulkanInterface::CreateTextureSampler()
{
    for (size_t i = 0; i < textureFiles.size(); i++)
    {
        ...
        textureSamplers.push_back(currentSampler);
...
void VulkanInterface::CreateTextureImageView() {
    for (size_t i = 0; i < textureFiles.size(); i++)
    {
        VkImageView currentView = CreateImageView(textureImages[i], VK_FORMAT_R8G8B8A8_SRGB, VK_IMAGE_ASPECT_COLOR_BIT);

        textureImageViews.push_back(currentView);
    }
}
...
void VulkanInterface::CreateTextureImage() {
    for (size_t i = 0; i < textureFiles.size(); i++)
    {
        ...
        VmaAllocation currentImageMemory;
        textureImagesMemory.push_back(currentImageMemory);
...
void VulkanInterface::CreateImage(uint32_t width, uint32_t height, VkFormat format, VkImageTiling tiling, VkImageUsageFlags usage, VkMemoryPropertyFlags properties, VkImage& image, VmaAllocation& imageMemory) {
    ...
    if (vmaCreateImage(allocator, &imageInfo, &allocationInfo, &image, &imageMemory, nullptr) != VK_SUCCESS)
    {
...
VulkanInterface::QueueFamilyIndices VulkanInterface::FindQueueFamilies(VkPhysicalDevice device) {
...
VulkanInterface::SwapChainSupportDetails VulkanInterface::QuerySwapChainSupport(VkPhysicalDevice device) {
...
void VulkanInterface::UpdateUniformBuffer(uint32_t currentImage) {
    ...
    globalInfo.proj = glm::perspective(glm::radians(m_camera->Zoom), swapChainExtent.width / (float)swapChainExtent.height, 0.1f, 10000.0f);
...
void VulkanInterface::Cleanup() {
    ...
    for (size_t i = 0; i < textureFiles.size(); i++)
    {
        ...
        vmaDestroyImage(allocator, textureImages[i], textureImagesMemory[i]);
